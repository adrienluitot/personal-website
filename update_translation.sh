...
if [ "$all" = true ] || [ "$updateMain" = true ]; then
    ...
    static-i18n -l en -i en -i fr --allowHtml --exclude projects --outputDir site html_templates
fi

if [ "$all" = true ] || [ "$updateProjects" = true ]; then
    echo "[Debug] Translate projects"
    static-i18n -l en -i en -i fr --allowHtml --exclude index.html --fixPaths false --outputDir site html_templates
fi
